...
class TestStyles < Test::Unit::TestCase
  ...
  def test_add_style_border_edges
    @styles.add_style border: { style: :thin, color: '0000FFFF', edges: %i[top bottom] }
    parts = @styles.borders.last.prs
    parts.each { |pr| assert_equal(pr.color.rgb, '0000FFFF', "Style is applied to #{pr.name} properly") }
    assert((parts.map { |pr| pr.name.to_s }.sort && %w[bottom top]).size == 2, 'specify two edges, and you get two border prs')
  end
  ...
  def test_parse_border_basic_options
    b_opts = { border: { diagonalUp: 1, edges: %i[left right], color: 'FFDADADA', style: :thick } }
  ...
  def test_parse_border_options_integer_dxf
    b_opts = { border: { edges: %i[left right], color: 'FFFFFFFF', style: :thick } }
    b = @styles.parse_border_options(b_opts)
