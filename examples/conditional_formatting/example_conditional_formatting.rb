...
book.add_worksheet(name: 'Cell Is') do |ws|
  ...
  offset.upto(rows + offset) do |i|
    ws.add_row ["Q#{i}", 10_000 * ((rows / 2 - i) * (rows / 2 - i)), "=100*B#{i}/SUM(B3:B#{rows + offset})"], style: [nil, money, percent]
  end
...
book.add_worksheet(name: 'Contains Text') do |ws|
  ...
  rows = 20
  offset.upto(rows + offset) do |i|
    ws.add_row ["Q#{i}", 10_000 * ((rows / 2 - i) * (rows / 2 - i)), "=100*B#{i}/SUM(B3:B#{rows + offset})", (10_000 * ((rows / 2 - i) * (rows / 2 - i))) > 100_000 ? 'PROFIT' : 'LOSS'], style: [nil, money, percent, status]
  end
