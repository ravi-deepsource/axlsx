...
if examples.include? :custom_borders
  wb.styles do |s|
    red_border = s.add_style border: { style: :thick, color: 'FFFF0000', edges: %i[left right] }
...
if examples.include? :deep_custom_borders
  wb.styles do |s|
    top_bottom = s.add_style border: { style: :thick, color: 'FFFF0000', edges: %i[top bottom] }
...
if examples.include? :outline_level
  wb.add_worksheet(name: 'outline_level') do |sheet|
    sheet.add_row [1, 2, 3, Time.now, 5, 149_455.15]
    sheet.add_row [1, 2, 5, 6, 5, 14_100.19]
    sheet.add_row [9_500_002_267, 1212, 1212, 5, 14_100.19]
...
if examples.include? :images
  wb.add_worksheet(name: 'Image with Hyperlink') do |sheet|
    img = File.expand_path('image1.jpeg', __dir__)
...
if examples.include? :auto_filter
  wb.add_worksheet(name: 'Auto Filter') do |sheet|
    ...
    sheet.add_row %w[Build Duration Finished Rvm]
...
if examples.include? :data_types
  wb.add_worksheet(name: 'Automatic cell types') do |sheet|
    ...
    sheet.add_row %w[Date Time String Boolean Float Integer]
...
if examples.include? :number_currency_format
  wb.add_worksheet(name: 'Formats and Currency') do |sheet|
    ...
    sheet.add_row [1500, -122.34, 123_456_789, 594_829], style: [currency, red_negative, comma, super_funk]
  end
end
...
if examples.include? :bar_chart
  wb.add_worksheet(name: 'Bar Chart') do |sheet|
    ...
    sheet.add_chart(Axlsx::Bar3DChart, start_at: 'A6', end_at: 'F20') do |chart|
      chart.add_series data: sheet['B2:B4'], labels: sheet['A2:A4'], title: sheet['A1'], colors: %w[00FF00 0000FF]
    end
  end
end
...
if examples.include? :chart_gridlines
  wb.add_worksheet(name: 'Chart With No Gridlines') do |sheet|
    ...
    %w[first second third].each { |label| sheet.add_row [label, rand(1..24)] }
    sheet.add_chart(Axlsx::Bar3DChart, start_at: 'A6', end_at: 'F20') do |chart|
      chart.add_series data: sheet['B2:B4'], labels: sheet['A2:A4'], colors: %w[00FF00 FF0000]
...
if examples.include? :line_chart
  wb.add_worksheet(name: 'Line Chart') do |sheet|
    ...
    4.times do
      sheet.add_row [rand(1..24), rand(1..24)]
    end
...
if examples.include? :hide_gridlines
  wb.add_worksheet(name: 'No Gridlines') do |sheet|
    sheet.add_row %w[This Sheet Hides Gridlines]
...
if examples.include? :panes
  wb.add_worksheet(name: 'panes') do |sheet|
    ...
    100.times { |index| sheet << ['row header', (0..index).to_a].flatten }
...
if examples.include? :conditional_formatting
  ...
  wb.add_worksheet(name: 'Conditional Cell Is') do |sheet|
    ...
    offset.upto(rosheet + offset) do |i|
      sheet.add_row ["Q#{i}", 10_000 * ((rosheet / 2 - i) * (rosheet / 2 - i)), "=100*B#{i}/SUM(B3:B#{rosheet + offset})"], style: [nil, money, percent]
    end
  ...
  wb.add_worksheet(name: 'Conditional Data Bar') do |sheet|
    ...
    offset.upto(rows + offset) do |i|
      sheet.add_row ["Q#{i}", 10_000 * ((rows / 2 - i) * (rows / 2 - i)), "=100*B#{i}/SUM(B3:B#{rows + offset})"], style: [nil, money, percent]
    end
    # Apply conditional formatting to range B3:B100 in the worksheet
  ...
  wb.add_worksheet(name: 'Contains Text') do |sheet|
    ...
    rows = 20
    offset.upto(rows + offset) do |i|
      sheet.add_row ["Q#{i}", 10_000 * ((rows / 2 - i) * (rows / 2 - i)), "=100*B#{i}/SUM(B3:B#{rows + offset})", (10_000 * ((rows / 2 - i) * (rows / 2 - i))) > 100_000 ? 'PROFIT' : 'LOSS'], style: [nil, money, percent, status]
    end
