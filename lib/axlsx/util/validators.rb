module Axlsx
  ...
  class RegexValidator
    ...
    def self.validate(_name, regex, v)
      raise ArgumentError, format(ERR_REGEX, v.inspect, regex.to_s) unless v.respond_to?(:to_s) && v.to_s.match(regex)
    end
  end
  ...
  class DataTypeValidator
    ...
    def self.validate(name, types, v, other = false)
      if other.is_a?(Proc)
        raise ArgumentError, format(ERR_TYPE, v.inspect, name, types.inspect) unless other.call(v)
      end
  ...
  def self.validate_page_orientation(v)
    RestrictionValidator.validate 'page_orientation', %i[default landscape portrait], v
  end

  # Requires that the value is one of :none, :single, :double, :singleAccounting, :doubleAccounting
  def self.validate_cell_u(v)
    RestrictionValidator.validate 'cell run style u', %i[none single double singleAccounting doubleAccounting], v
  end
  ...
  def self.validate_pattern_type(v)
    RestrictionValidator.validate :pattern_type, %i[none solid mediumGray darkGray lightGray darkHorizontal darkVertical darkDown darkUp darkGrid
  ...
  def self.validate_time_period_type(v)
    RestrictionValidator.validate :time_period_type, %i[today yesterday tomorrow last7Days thisMonth lastMonth nextMonth thisWeek lastWeek nextWeek], v
  end
  ...
  def self.validate_icon_set(v)
    RestrictionValidator.validate :iconSet, %w[3Arrows 3ArrowsGray 3Flags 3TrafficLights1 3TrafficLights2 3Signs 3Symbols 3Symbols2 4Arrows 4ArrowsGray 4RedToBlack 4Rating 4TrafficLights 5Arrows 5ArrowsGray 5Rating 5Quarters], v
  end
  ...
  def self.validate_conditional_formatting_type(v)
    RestrictionValidator.validate :conditional_formatting_type, %i[expression cellIs colorScale dataBar iconSet top10 uniqueValues duplicateValues containsText notContainsText beginsWith endsWith containsBlanks notContainsBlanks containsErrors notContainsErrors timePeriod aboveAverage], v
  end
  ...
  def self.validate_conditional_formatting_value_object_type(v)
    RestrictionValidator.validate :conditional_formatting_value_object_type, %i[num percent max min formula percentile], v
  end
  ...
  def self.validate_conditional_formatting_operator(v)
    RestrictionValidator.validate :conditional_formatting_type, %i[lessThan lessThanOrEqual equal notEqual greaterThanOrEqual greaterThan between notBetween containsText notContains beginsWith endsWith], v
  end
  ...
  def self.validate_gradient_type(v)
    RestrictionValidator.validate :gradient_type, %i[linear path], v
  end
  ...
  def self.validate_scatter_style(v)
    Axlsx::RestrictionValidator.validate 'ScatterChart.scatterStyle', %i[none line lineMarker marker smooth smoothMarker], v.to_sym
  end
  ...
  def self.validate_horizontal_alignment(v)
    RestrictionValidator.validate :horizontal_alignment, %i[general left center right fill justify centerContinuous distributed], v
  end
  ...
  def self.validate_vertical_alignment(v)
    RestrictionValidator.validate :vertical_alignment, %i[top center bottom justify distributed], v
  end
  ...
  def self.validate_table_element_type(v)
    RestrictionValidator.validate :table_element_type, %i[wholeTable headerRow totalRow firstColumn lastColumn firstRowStripe secondRowStripe firstColumnStripe secondColumnStripe firstHeaderCell lastHeaderCell firstTotalCell lastTotalCell firstSubtotalColumn secondSubtotalColumn thirdSubtotalColumn firstSubtotalRow secondSubtotalRow thirdSubtotalRow blankRow firstColumnSubheading secondColumnSubheading thirdColumnSubheading firstRowSubheading secondRowSubheading thirdRowSubheading pageFieldLabels pageFieldValues], v
  end
  ...
  def self.validate_data_validation_error_style(v)
    RestrictionValidator.validate :validate_data_validation_error_style, %i[information stop warning], v
  end
  ...
  def self.validate_data_validation_operator(v)
    RestrictionValidator.validate :data_validation_operator, %i[lessThan lessThanOrEqual equal notEqual greaterThanOrEqual greaterThan between notBetween], v
  end
  ...
  def self.validate_data_validation_type(v)
    RestrictionValidator.validate :data_validation_type, %i[custom data decimal list none textLength time whole], v
  end
  ...
  def self.validate_sheet_view_type(v)
    RestrictionValidator.validate :sheet_view_type, %i[normal page_break_preview page_layout], v
  end
  ...
  def self.validate_pane_type(v)
    RestrictionValidator.validate :active_pane_type, %i[bottom_left bottom_right top_left top_right], v
  end
  ...
  def self.validate_split_state_type(v)
    RestrictionValidator.validate :split_state_type, %i[frozen frozen_split split], v
  end
  ...
  def self.validate_display_blanks_as(v)
    RestrictionValidator.validate :display_blanks_as, %i[gap span zero], v
  end

  # Requires that the value is one of :visible, :hidden, :very_hidden
  def self.validate_view_visibility(v)
    RestrictionValidator.validate :visibility, %i[visible hidden very_hidden], v
  end
  ...
  def self.validate_marker_symbol(v)
    RestrictionValidator.validate :marker_symbol, %i[default circle dash diamond dot picture plus square star triangle x], v
  end
end
